...
from account_manager import (
    ...
    check_account, listen_for_codes
)
...
async def menu_accounts(cq: CallbackQuery):
    ...
    await cq.message.edit_text(
        f"📱 <b>Аккаунты</b>\n\nВсего: <b>{count}</b>",
        reply_markup=kb.accounts_menu(accs), parse_mode="HTML"
    )
...
async def _get_phone_from_session(session_name: str) -> str:
    ...
    try:
        ...
        return f"+{me.phone_number}" if me and me.phone_number else None
    except Exception:
        return None
...
async def admin_user_detail(cq: CallbackQuery):
    if not is_admin(cq.from_user.id):
        return await cq.answer("⛔ Нет доступа", show_alert=True)
    ...
    accs_list = [dict(a) for a in accs]
    text = (
        f"👤 <b>Пользователь</b> <code>{target_uid}</code>\n"
        f"📱 Аккаунтов: <b>{len(accs_list)}</b>\n\n"
        "Выберите аккаунт для управления:"
    )
    await cq.message.edit_text(
        text, reply_markup=kb.admin_user_detail(target_uid, accs_list), parse_mode="HTML"
    )
...
async def adm_check(cq: CallbackQuery):
    ...
    result = await check_account(acc["session_file"])
...
async def adm_clearchats(cq: CallbackQuery):
    ...
    if not acc:
        return
    await cq.answer("⏳")
    result = await clear_chats(acc["session_file"], acc.get("two_fa_password"))
...
async def adm_clearcontacts(cq: CallbackQuery):
    ...
    if not acc:
        return
    await cq.answer("⏳")
    result = await clear_contacts(acc["session_file"], acc.get("two_fa_password"))
...
async def adm_kick(cq: CallbackQuery):
    ...
    if not acc:
        return
    await cq.answer("⏳")
    result = await kick_all_sessions(acc["session_file"], acc.get("two_fa_password"))
...
async def adm_leavebots(cq: CallbackQuery):
    ...
    if not acc:
        return
    await cq.answer("⏳")
    result = await leave_all_bots(acc["session_file"], acc.get("two_fa_password"))
...
async def adm_download(cq: CallbackQuery, bot: Bot):
    ...
    if not acc:
        return
    await cq.answer("⏳")
    result = await download_session(acc["session_file"])
    if result.get("ok") and result.get("path"):
        await bot.send_document(cq.from_user.id, FSInputFile(result["path"]))
    else:
        await cq.message.answer(f"❌ {result.get('error', 'Ошибка')}")
...
async def adm_getcode(cq: CallbackQuery):
    ...
    result = await listen_for_codes(acc["session_file"], acc.get("two_fa_password"))
    if result.get("ok"):
        await cq.message.edit_text(
            f"📲 <b>Код получен:</b> <code>{result['code']}</code>",
            reply_markup=kb.admin_acc_menu(acc_id, owner_uid), parse_mode="HTML"
        )
    else:
        await cq.message.edit_text(
