...
async def upload_session_file(msg: Message, state: FSMContext, bot: Bot):
    ...
    # .session файл — используем напрямую
    if fname.endswith(".session"):
        # Имя файла = номер телефона если возможно
        phone = fname.replace(".session", "")
        session_path = os.path.join("sessions", f"{uid}_{phone}.session")
        os.rename(tmp_path, session_path)
        await add_account(uid, phone or f"acc_{uid}", session_path)
        await state.clear()
        return await msg.answer(
    ...
    # .zip с tdata — распаковываем и конвертируем
        ...
        # Пробуем конвертировать tdata → session через opentele
        ...
        try:
            ...
            session_path = os.path.join("sessions", f"{uid}_tdata.session")
            ...
            os.rename(session_path + ".session" if os.path.exists(session_path + ".session") else session_path, final_path)
            ...
            return await msg.answer(
                f"✅ <b>tdata конвертирована и добавлена!</b>\n\n📞 +{phone}",
                reply_markup=kb.back_btn("menu_accounts"), parse_mode="HTML"
            )

        except ImportError:
            # opentele не установлен — сохраняем tdata как есть
            await add_account(uid, f"tdata_{uid}", extract_dir)
            await state.clear()
            return await msg.answer(
                "⚠️ <b>tdata сохранена, но не сконвертирована</b>\n\n"
                "Для конвертации нужна библиотека <code>opentele</code>.\n"
