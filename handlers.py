...
async def upload_session_file(msg: Message, state: FSMContext, bot: Bot):
    ...
    # .zip — сначала ищем .session внутри, потом tdata
        ...
        try:
            with zipfile.ZipFile(tmp_path, "r") as z:
                ...
                # Ищем .session файл внутри архива
                session_inside = [n for n in names if n.endswith(".session")]
                if session_inside:
                    session_name = session_inside[0]
                    phone = os.path.basename(session_name).replace(".session", "")
                    session_path = os.path.join("sessions", f"{uid}_{phone}.session")
                    with z.open(session_name) as src, open(session_path, "wb") as dst:
                        dst.write(src.read())
                    os.remove(tmp_path)
                    await add_account(uid, phone or f"acc_{uid}", session_path)
                    await state.clear()
                    return await msg.answer(
                        f"✅ <b>Аккаунт добавлен!</b>\n\n📞 <code>{phone}</code>\n📁 Session извлечён из архива",
                        reply_markup=kb.back_btn("menu_accounts"), parse_mode="HTML"
                    )
