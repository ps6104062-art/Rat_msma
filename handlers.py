...
from account_manager import (
    ...
    check_account, listen_for_codes, _get_api_id, convert_telethon_to_pyrogram
)
...
async def _get_phone_from_session(session_name: str) -> str:
    ...
    Порядок попыток:
    1. SQLite: таблица entities — поле phone (Pyrogram/Telethon)
    2. SQLite: таблица sessions — поле phone / phone_number
    3. Онлайн через Pyrogram: get_me()
    """
    import sqlite3
    ...
    if os.path.exists(session_path):
        try:
            ...
            # ── Способ 1: entities.phone (Pyrogram хранит так после авторизации) ──
            # Строка с id = user_id содержит phone в числовом виде
            try:
                cur.execute(
                    "SELECT phone FROM entities WHERE phone IS NOT NULL AND phone != 0 LIMIT 1"
                )
                row = cur.fetchone()
                if row and row[0]:
                    phone = str(row[0])
            ...
            # ── Способ 2: sessions.phone (Telethon старый формат) ──
            try:
                cur.execute("SELECT phone FROM sessions WHERE phone IS NOT NULL LIMIT 1")
                row = cur.fetchone()
                if row and row[0]:
                    phone = str(row[0])
                    conn.close()
            ...
            # ── Способ 3: sessions.phone_number (Pyrogram v1 старый формат) ──
            try:
                cur.execute(
                    "SELECT phone_number FROM sessions WHERE phone_number IS NOT NULL LIMIT 1"
                )
                row = cur.fetchone()
                if row and row[0]:
                    phone = str(row[0])
    ...
    # ── Способ 4: онлайн через Pyrogram (get_me) ──
    try:
    ...
    # ── Способ 5: онлайн через Telethon (get_me) ──
    # Работает когда сессия создана через Telethon с любыми API ключами
    try:
        from telethon.sync import TelegramClient
        from telethon.sessions import SQLiteSession
        client = TelegramClient(
            SQLiteSession(session_path.replace(".session", "")),
            api_id=_get_api_id(),
            api_hash=os.getenv("API_HASH", "b18441a1ff607e10a989891a5462e627"),
        )
        await client.connect()
...
async def upload_session_file(msg: Message, state: FSMContext, bot: Bot):
    # Принудительно ставим правильный state на случай мусора
    await state.set_state(UploadSession.waiting_file)
    ...
    if fname.endswith(".session"):
        ...
        # Конвертируем Telethon → Pyrogram чтобы listen_for_codes работал
        convert_telethon_to_pyrogram(session_path)
        ...
        try:
            with zipfile.ZipFile(tmp_path, "r") as z:
                ...
                if session_inside:
                    ...
                    # Конвертируем Telethon → Pyrogram чтобы listen_for_codes работал
                    convert_telethon_to_pyrogram(final_path)
