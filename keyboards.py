...
def accounts_menu(accounts: list) -> InlineKeyboardMarkup:
    ...
    if accounts:
        kb.row(
            InlineKeyboardButton(text="✅ Выбрать все", callback_data="acc_select_all"),
            InlineKeyboardButton(text="☑️ Выбрать", callback_data="acc_select_one"),
        )
        kb.row(
            InlineKeyboardButton(text="➕ По номеру", callback_data="acc_add"),
            InlineKeyboardButton(text="📁 Файлом (.session)", callback_data="acc_upload"),
        )
        kb.row(
...
def account_list(accounts: list, selected: list = None, multi=False) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    selected = selected or []

    for acc in accounts:
        check = "✅ " if acc["id"] in selected else "○ "
        cb = f"acc_toggle_{acc['id']}" if multi else f"acc_open_{acc['id']}"
    ...
    if multi and selected:
        kb.row(InlineKeyboardButton(
            text=f"⚡ Действия ({len(selected)} акк.)",
