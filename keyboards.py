...
def main_menu(user_id: int = None) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="⚙️ Настройки", callback_data="menu_settings"))
...
def single_account_menu(acc_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🛡 Проверить статус", callback_data=f"do_check_{acc_id}"))
    kb.row(
    ...
    kb.row(InlineKeyboardButton(text="📲 Получить код", callback_data=f"do_get_code_{acc_id}"))
    kb.row(InlineKeyboardButton(text="❌ Удалить аккаунт", callback_data=f"do_delete_acc_{acc_id}"))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="acc_list"))
    return kb.as_markup()
...
def mass_actions_menu(selected: list) -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🛡 Проверить статус", callback_data=f"mass_check|{ids}"))
    kb.row(
...
def subscribe_menu(channels: list) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for ch in channels:
        title = ch["title"] or ch["channel_url"]
        kb.row(InlineKeyboardButton(text=f"📢 {title}", url=ch["channel_url"]))
    kb.row(InlineKeyboardButton(text="✅ Я подписался", callback_data="check_subscribe"))
    return kb.as_markup()
...
def admin_menu() -> InlineKeyboardMarkup:
    ...
    kb.row(
        ...
        InlineKeyboardButton(text="📱 Все аккаунты", callback_data="admin_accs_0"),
    )
    kb.row(
...
def admin_user_detail_menu(target_uid: int, is_banned: bool, accs_count: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    if is_banned:
        kb.row(InlineKeyboardButton(text="✅ Разбанить", callback_data=f"admin_unban_{target_uid}"))
    else:
        kb.row(InlineKeyboardButton(text="🚫 Забанить", callback_data=f"admin_ban_{target_uid}"))
    kb.row(InlineKeyboardButton(text="✉️ Написать", callback_data=f"admin_msg_{target_uid}"))
    if accs_count > 0:
        kb.row(InlineKeyboardButton(text=f"📱 Аккаунты ({accs_count})", callback_data=f"admin_user_accs_{target_uid}"))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="admin_users_0"))
    return kb.as_markup()

def admin_user_accs_menu(target_uid: int, accs: list) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for acc in accs:
    ...
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data=f"admin_user_{target_uid}"))
    return kb.as_markup()

def admin_all_accs_menu(accs: list, offset: int = 0) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for acc in accs:
        label = acc["phone"] or f"#{acc['id']}"
        kb.row(InlineKeyboardButton(
            text=f"📱 {label} (uid:{acc['user_id']})",
            callback_data=f"admin_acc_{acc['id']}"
        ))
    nav = []
    if offset > 0:
        nav.append(InlineKeyboardButton(text="◀️", callback_data=f"admin_accs_{offset-20}"))
    if len(accs) == 20:
        nav.append(InlineKeyboardButton(text="▶️", callback_data=f"admin_accs_{offset+20}"))
    if nav:
        kb.row(*nav)
...
def admin_acc_menu(acc_id: int, owner_uid: int) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data=f"admin_acc_{acc_id}"))
    return kb.as_markup()

def admin_banner_menu() -> InlineKeyboardMarkup:
    """Выбор типа баннера."""
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🏠 Баннер главного меню", callback_data="admin_banner_main"))
    kb.row(InlineKeyboardButton(text="📱 Баннер раздела «Аккаунты»", callback_data="admin_banner_accounts"))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="admin_panel"))
    return kb.as_markup()

def admin_banner_single_menu(banner_type: str, has_banner: bool) -> InlineKeyboardMarkup:
    """Меню управления конкретным баннером."""
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🖼 Загрузить/сменить фото", callback_data=f"admin_banner_set_{banner_type}"))
    if has_banner:
        kb.row(InlineKeyboardButton(text="🗑 Удалить баннер", callback_data=f"admin_banner_del_{banner_type}"))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="admin_banner"))
    return kb.as_markup()

def admin_channels_menu(channels: list) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for ch in channels:
        title = ch["title"] or ch["channel_url"]
        kb.row(InlineKeyboardButton(
            text=f"❌ {title}",
            callback_data=f"admin_ch_del_{ch['id']}"
        ))
    if len(channels) < 5:
        kb.row(InlineKeyboardButton(text="➕ Добавить канал", callback_data="admin_ch_add"))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="admin_panel"))
    return kb.as_markup()
