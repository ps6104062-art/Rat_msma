...
async def listen_for_codes(phone: str, session_file: str, callback, timeout: int = 300):
    ...
    # Используем no_updates=False чтобы получать апдейты
    session_name = session_file or os.path.join(SESSIONS_DIR, phone.replace("+", ""))
    client = Client(
        name=session_name,
        api_id=_get_api_id(),
        api_hash=os.getenv("API_HASH", "b18441a1ff607e10a989891a5462e627"),
        no_updates=False,  # Важно! Иначе сообщения не придут
    )
    received = []
    ...
    try:
        await client.start()
        # Слушаем ВСЕ приватные сообщения — фильтр по user() может не сработать
        # если Telegram шлёт код через служебный чат а не юзера
