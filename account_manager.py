...
# Кэш клиентов между send_code и sign_in (ключ = номер телефона)
_pending_clients: dict = {}


def _get_api_id() -> int:
    """Возвращает API_ID из env, проверяя что он помещается в 32-битный int."""
    raw = os.getenv("API_ID", "2040")
    val = int(raw)
    if val > 2147483647:
        raise ValueError(
            f"API_ID={val} слишком большой (max 2147483647). "
            "Получите корректный API_ID на https://my.telegram.org"
        )
    return val
...
async def send_code(phone: str) -> dict:
    """Отправляет код подтверждения на номер и сохраняет клиент для sign_in."""
    # Если уже был клиент — отключаем
    old = _pending_clients.pop(phone, None)
    if old:
        try:
    ...
    try:
        ...
        # Сохраняем клиент — он понадобится в sign_in
...
async def sign_in(phone: str, code: str, phone_code_hash: str, password: str = None) -> dict:
    """Авторизует аккаунт по коду, используя сохранённый клиент."""
    session_name = os.path.join(SESSIONS_DIR, phone.replace("+", ""))

    # Берём сохранённый клиент (тот же, что отправлял код)
...
def _is_telethon_session(session_file: str) -> bool:
    """Определяет формат сессии по структуре SQLite БД.
    True = Telethon, False = Pyrogram."""
    import sqlite3
    ...
    try:
        ...
        conn.close()
        # Pyrogram имеет tmp_auth_key, Telethon — нет
        return "tmp_auth_key" not in cols and "auth_key" in cols
    except Exception:
        return False


async def _listen_telethon(session_file: str, callback, timeout: int) -> dict:
    """Слушает сообщения через Telethon (для Telethon-сессий)."""
    from telethon import TelegramClient, events

    api_id = _get_api_id()
    api_hash = os.getenv("API_HASH", "b18441a1ff607e10a989891a5462e627")
    # Telethon принимает путь без .session
    session_path = session_file[:-8] if session_file.endswith(".session") else session_file
    ...
    try:
        ...
        if not await client.is_user_authorized():
            await client.disconnect()
            return {"ok": False, "error": "Сессия не авторизована (Telethon)"}
...
async def _listen_pyrogram(session_file: str, callback, timeout: int) -> dict:
    """Слушает сообщения через Pyrogram (для Pyrogram-сессий)."""
...
async def listen_for_codes(phone: str, session_file: str, callback, timeout: int = 300):
    ...
    Автоматически определяет формат сессии (Telethon или Pyrogram) по структуре БД."""
    session_name = session_file or os.path.join(SESSIONS_DIR, phone.replace("+", ""))

    if _is_telethon_session(session_name):
        return await _listen_telethon(session_name, callback, timeout)
    else:
        return await _listen_pyrogram(session_name, callback, timeout)
...
def convert_telethon_to_pyrogram(session_path: str) -> bool:
    """Конвертирует Telethon .session в Pyrogram-совместимый формат на месте.
    Возвращает True если успешно."""
    import sqlite3

    path = session_path if session_path.endswith(".session") else session_path + ".session"
    if not os.path.exists(path):
        return False
    ...
    try:
        ...
        # Проверяем формат — Pyrogram имеет колонку tmp_auth_key
        cur.execute("PRAGMA table_info(sessions)")
        cols = [row[1] for row in cur.fetchall()]
        ...
        # Добавляем недостающие Pyrogram колонки
        try:
            cur.execute("ALTER TABLE sessions ADD COLUMN takeout_id INTEGER")
        except Exception:
            pass
        try:
        ...
        # Записываем пустой tmp_auth_key
        cur.execute("UPDATE sessions SET tmp_auth_key = ? WHERE dc_id = ?", (b"", dc_id))

        # Таблица version нужна Pyrogram
        cur.execute("CREATE TABLE IF NOT EXISTS version (number INTEGER PRIMARY KEY)")
        cur.execute("INSERT OR IGNORE INTO version VALUES (1)")
